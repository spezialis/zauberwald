...
function setup() {
  ...
  frameRate(5);
  gol = new GOL(); // gol.init();

  video = createCapture(VIDEO);
  video.size(sWidth, sHeight); // Create a new poseNet method with a single detection

  poseNet = ml5.poseNet(video, modelReady); // This sets up an event that fills the global variable "poses"
...
function draw() {
  background(128);
  gol.generate();
  gol.display(); // image(video, 0, 0, sWidth, sHeight);
  // Draw the nose

  drawNose(); // Start elements on body part

  gol.drawOnBodyPart(0); // console.log(drawOnBodyPart());
} //
// Game of Life ––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––
//
...
function drawKeypoints() {
  ...
  for (let i = 0; i < poses.length; i++) {
    ...
    for (let j = 0; j < pose.keypoints.length; j++) {
      ...
      let keypoint = pose.keypoints[j]; // Only draw an ellipse is the pose probability is bigger than 0.2
...
function displayImg() {
  for (let i = 0; i < poses.length; i++) {
    // For each pose detected, loop through all the keypoints
    let pose = poses[i].pose; // Displays the image at point (0, height/2) at half size
...
function logPosePosition(bodyPart) {
  for (let i = 0; i < poses.length; i++) {
    // For each pose detected, loop through all the keypoints
    let pose = poses[i].pose; // let position = createVector(pose.keypoints[bodyPart].position.x, pose.keypoints[bodyPart].position.y)

    if (pose.keypoints[bodyPart]) {
      return {
        x: pose.keypoints[bodyPart].position.x,
        y: pose.keypoints[bodyPart].position.y
      };
    } else {
      return;
